...
export async function getTotalReceitaEventos(
  ...
  if (organizadorId === undefined) return null;
  if (periodo) {
    return await prisma.$queryRaw`SELECT SUM(a.valor) valor FROM pedidos a join eventos b on b.id = a.eventoId and b.organizadorId = ${organizadorId} WHERE a.status = "Pago" and a.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY);`;
  }
...
export async function getTotalVagasEventosPeriodo(
  ...
  return await prisma.$queryRaw`select SUM(a.vagas) as vagas from eventos a where a.organizadorId = ${organizadorId} and a.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY);`;
}
...
export async function getTotalTicketsVendidos(
  ...
  if (organizadorId === undefined) return null;
  if (periodo) {
    return await prisma.$queryRaw`select IFNULL(SUM(b.quantidade),0) as vendidos from eventos a join pedidos b on a.id = b.eventoId where b.status = "Pago" and a.organizadorId = ${organizadorId} and b.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY);`;
  }
...
export async function getMelhorEvento(
  ...
  if (organizadorId === undefined) return null;
  if (periodo) {
    return await prisma.$queryRaw`select a.titulo, IFNULL(SUM(b.quantidade),0) as vendidos from eventos a join pedidos b on a.id = b.eventoId where b.status = "Pago" and a.organizadorId = ${organizadorId} and b.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY) GROUP BY a.titulo ORDER BY vendidos DESC LIMIT 1;`;
  }
...
export async function getTabelaGeralEventos(
  ...
  if (organizadorId === undefined) return null;
  if (periodo) {
    return await prisma.$queryRaw`select a.titulo, b.created_at, b.formaPagamento, c.descricao, b.status, b.valor, b.quantidade from eventos a join pedidos b on a.id = b.eventoId join tipoTickets c on b.tipoTicketId = c.id where organizadorId = ${organizadorId} and b.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY)`;
  }
...
export async function getEventosGraficoPeriodo(
  ...
  if (organizadorId === undefined || periodo === undefined) return null;
  return await prisma.$queryRaw`select distinct a.id, a.titulo from eventos a join pedidos b on a.id = b.eventoId where organizadorId = ${organizadorId} and b.created_at >= DATE_SUB(CURDATE(), INTERVAL ${periodo}-1 DAY) order by a.id`;
}
